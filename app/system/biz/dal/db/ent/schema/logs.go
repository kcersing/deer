...
func (Logs) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Bool("success").Comment("success of log | 日志请求是否成功").Optional(),
		...
		field.String("operatorsr").Optional().Comment("operator of log | 日志操作者").Optional(),
		field.Int64("time").Optional().Comment("time of log(millisecond) | 日志时间(毫秒)").Optional(),
		field.Int64("identity").Optional().Comment("1会员2员工 | 身份").Optional(),
	}
}
