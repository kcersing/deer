...
func (_q *ItemQuery) QueryProduct() *ProductQuery {
...
func (_q *ItemQuery) WithProduct(opts ...func(*ProductQuery)) *ItemQuery {
	query := (&ProductClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Item, error) {
	...
	if query := _q.withProduct; query != nil {
		if err := _q.loadProduct(ctx, query, nodes,
			...
			func(n *Item, e *Product) { n.Edges.Product = append(n.Edges.Product, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ItemQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*Item, init func(*Item), assign func(*Item, *Product)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(item.ProductTable)
		s.Join(joinT).On(s.C(product.FieldID), joinT.C(item.ProductPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(item.ProductPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(item.ProductPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
