...
func (_q *ProductQuery) QueryItems() *ProductItemQuery {
	query := (&ProductItemClient{config: _q.config}).Query()
...
func (_q *ProductQuery) WithItems(opts ...func(*ProductItemQuery)) *ProductQuery {
	query := (&ProductItemClient{config: _q.config}).Query()
...
func (_q *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := _q.withItems; query != nil {
		if err := _q.loadItems(ctx, query, nodes,
			func(n *Product) { n.Edges.Items = []*ProductItem{} },
			func(n *Product, e *ProductItem) { n.Edges.Items = append(n.Edges.Items, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ProductQuery) loadItems(ctx context.Context, query *ProductItemQuery, nodes []*Product, init func(*Product), assign func(*Product, *ProductItem)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(product.ItemsTable)
		s.Join(joinT).On(s.C(productitem.FieldID), joinT.C(product.ItemsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(product.ItemsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(product.ItemsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*ProductItem](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
