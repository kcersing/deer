...
func (_q *ProductItemQuery) QueryProduct() *ProductQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, productitem.ProductTable, productitem.ProductPrimaryKey...),
		)
...
func (_q *ProductItemQuery) WithProduct(opts ...func(*ProductQuery)) *ProductItemQuery {
	query := (&ProductClient{config: _q.config}).Query()
...
//		GroupBy(productitem.FieldCreatedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.ProductItem.Query().
//		Select(productitem.FieldCreatedAt).
//		Scan(ctx, &v)
...
func (_q *ProductItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ProductItem, error) {
	...
	if query := _q.withProduct; query != nil {
		if err := _q.loadProduct(ctx, query, nodes,
			func(n *ProductItem) { n.Edges.Product = []*Product{} },
			func(n *ProductItem, e *Product) { n.Edges.Product = append(n.Edges.Product, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ProductItemQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*ProductItem, init func(*ProductItem), assign func(*ProductItem, *Product)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*ProductItem)
	nids := make(map[int64]map[*ProductItem]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(productitem.ProductTable)
		s.Join(joinT).On(s.C(product.FieldID), joinT.C(productitem.ProductPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(productitem.ProductPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(productitem.ProductPrimaryKey[1]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*ProductItem]struct{}{byID[outValue]: {}}
