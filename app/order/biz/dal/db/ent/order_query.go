...
// QueryEventbus chains the current query on the "eventbus" edge.
func (_q *OrderQuery) QueryEventbus() *OrderEventsQuery {
	query := (&OrderEventsClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, order.EventbusTable, order.EventbusColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *OrderQuery) QuerySnapshots() *OrderSnapshotsQuery {
	query := (&OrderSnapshotsClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withEventbus:      _q.withEventbus.Clone(),
...
// WithEventbus tells the query-builder to eager-load the nodes that are connected to
// the "eventbus" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *OrderQuery) WithEventbus(opts ...func(*OrderEventsQuery)) *OrderQuery {
	query := (&OrderEventsClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withEventbus = query
	return _q
}
...
func (_q *OrderQuery) WithSnapshots(opts ...func(*OrderSnapshotsQuery)) *OrderQuery {
	query := (&OrderSnapshotsClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := _q.withEventbus; query != nil {
		if err := _q.loadEventbus(ctx, query, nodes,
			func(n *Order) { n.Edges.Eventbus = []*OrderEvents{} },
			func(n *Order, e *OrderEvents) { n.Edges.Eventbus = append(n.Edges.Eventbus, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *OrderQuery) loadEventbus(ctx context.Context, query *OrderEventsQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderEvents)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.OrderEvents(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.EventbusColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *OrderQuery) loadSnapshots(ctx context.Context, query *OrderSnapshotsQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderSnapshots)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
