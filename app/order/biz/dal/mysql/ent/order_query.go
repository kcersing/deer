...
func (_q *OrderQuery) QueryEvents() *OrderEventsQuery {
...
func (_q *OrderQuery) WithEvents(opts ...func(*OrderEventsQuery)) *OrderQuery {
	query := (&OrderEventsClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderQuery) loadEvents(ctx context.Context, query *OrderEventsQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderEvents)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
